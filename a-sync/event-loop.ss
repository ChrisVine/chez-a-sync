...
(define-record-type (event-loop _make-event-loop event-loop?)
  (fields (immutable mutex _mutex-get)
          (immutable q _q-get)
          (mutable mode _mode-get _mode-set!)
...
;; exception if it is applied to an event loop which is currently
;; running (prior to version 0.18, doing so would fail in a less
;; helpful way).
;;
...
(define (_event-loop-run-impl! el)
  ...
  ;; house-keeping, use 'try' not 'with-exception-handler' so as to
  ;; unwind the stack to ensure that any with-mutex forms have
  ;; unlocked by the time we enter the handler
  (try
   (let loop1 ()
     ;; we don't need to use the mutex in this procedure to access the
     ;; current-timeout field of an event loop object or any
	 ...
	 ;; event loop to unblock) and then run any events queued in
	 ;; the queue.  This (i) eliminates any concerns that events
	 ;; might go missing if the pipe fills up, and (ii) ensures
	 ;; that if a timeout or watch callback has posted an event
	 ;; (say ending the timeout or watch), that will have been
	 ;; acted on by the time the event loop begins its next
	 ;; iteration.
...
;; (await-put-bytevector! and await-put-string! in this library file,
;; and await-accept-ipv4-connection!, await-accept-ipv6-connection!
;; and await-accept-unix-connection! in the chez-simple-sockets
;; package) do not pass control to the event loop if they can operate
...
;; invoked on the event loop in an a-sync or compose-a-sync block in
;; order to bring it to an end, and (iii) before invoking this
;; procedure the a-sync or compose-a-sync block has done nothing
;; except make a call to one or more of those particular await-*
;; procedures, then in order to make sure the loop is running consider
;; calling await-yield! or some similar procedure before applying this
;; procedure.
;;
...
;; textual port.
;;
;; This procedure bypasses any output buffering in 'port' entirely,
;; and sends the output to the underlying file descriptor directly.
;; This is to enable this procedure to be used conveniently with
;; sockets: in chez scheme ports can be constructed from file
;; descriptors using the open-fd-input-port, open-fd-output-port and
;; open-fd-input/output-port procedures, and the last of those would
;; be useful for sockets except that chez scheme's port implementation
...
;; 'port' means that any buffered output port which has previously
;; been used for writing using chez scheme's R6RS write procedures
;; must be flushed before this procedure is called; but the best thing
;; is to carry out all writing to the port using this procedure or the
;; await-put-string! procedure, in which case all is good.  In the
;; case of a socket port, this can be enforced by constructing the
;; socket port with open-fd-input-port rather than
...
;; socket port (that is, one which is unseekable and has been
;; constructed with open-fd-input/output-port) is closed or otherwise
;; flushed for output, otherwise the exception mentioned above might
;; arise.
;;
