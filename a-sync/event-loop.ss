...
;; and await-put* procedures may be used for the port (but one
;; remaining point to watch out for is that clear-input-port must
;; normally be called before an input/output port representing a
;; socket is closed, otherwise the exception mentioned above might
;; arise by virtue of an implicit flush).  If using a binary port
;; constructed by 'open-fd-output-port' with buffering on or a textual
;; port constructed by that procedure with or without buffering, and
;; the port has previously been used for output by a procedure other
;; than c-write or an await-put* procedure, then it should be flushed
;; before this procedure is called.
;;
;; But the best thing with a socket is to carry out all writing to the
;; socket port using this procedure or the await-put-string!
;; procedure from the outset, in which case all is good.  This can be
;; enforced by constructing the socket port with open-fd-input-port
;; rather than open-fd-input/output-port.
;;
...
;; the port (but one remaining point to watch out for is that
;; clear-input-port must normally be called before an input/output
;; port representing a socket is closed, otherwise the exception
;; mentioned above might arise by virtue of an implicit flush).  If
;; using a port constructed by 'open-fd-output-port' which has
;; previously been used for output by a procedure other than c-write
;; or an await-put* procedure, then it should be flushed before this
;; procedure is called.
;;
;; But the best thing with a socket is to carry out all writing to the
